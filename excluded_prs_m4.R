...
system("Rscript PRSice.R --prsice PRSice_win64.exe --base summstats.txt --target mydata-excluded-A --binary-target T --pheno pheno1-A.txt --cov synPRS-nocov.covariate --stat BETA --no-clump --fastscore --print-snp --out PRSresults-m4A")

##COMPARACIÓ PRS X PHENO

prs <- read.table("PRSresults-m4A.best", header=T)
...
ggplot(subset_pheno_filtered, aes(x = factor(Pheno), y = PRS)) + 
  ...
  ggtitle("Boxplot of Synpatic PRS Score by Phenotype - Model 4") +
...
legend("right", legend = paste0("Synaptic PRS-Model 4: AUC = ", auc_value), bty="n")