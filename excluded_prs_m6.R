...
system("Rscript PRSice.R --prsice PRSice_win64.exe --base summstats.txt --target mydata-excluded --binary-target T --pheno pheno2.txt --cov synPRS.covariate --stat BETA --no-clump --fastscore --print-snp --out PRSresults-m6")

##COMPARACIÓ PRS X PHENO

prs <- read.table("PRSresults-m6.best", header=T)
...
ggplot(subset_pheno_filtered, aes(x = factor(Pheno), y = PRS)) + 
  ...
  ggtitle("Boxplot of Synpatic PRS Score by Phenotype - Model 6") +
...
ggplot(dat_filtered2, aes(x = Pheno, y = PRS, fill = factor(Sex))) + 
  ...
  ggtitle("Boxplot of Synpatic PRS Score by Phenotype and Sex - Model 6") +
  scale_x_discrete(labels = c("Control", "LOAD"))
...
legend("right", legend = paste0("Synaptic PRS-Model 6: AUC = ", auc_value), bty="n")
