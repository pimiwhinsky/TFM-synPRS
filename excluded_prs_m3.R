...
system("Rscript PRSice.R --prsice PRSice_win64.exe --base summstats.txt --target mydata-excluded-A --binary-target T --pheno pheno1-A.txt --cov synPRS-nocov.covariate --stat BETA --fastscore --print-snp --out PRSresults-m3A")

##COMPARACIÓ PRS X PHENO

prs <- read.table("PRSresults-m3A.best", header=T)
...
ggplot(subset_pheno_filtered, aes(x = factor(Pheno), y = PRS)) + 
  ...
  geom_jitter(position = position_jitter(width = 0.2), size = 3, shape = 4, alpha = 0.5) +  labs(x = "Phenotype", y = "Synaptic PRS Score") + 
  ggtitle("Boxplot of Synpatic PRS Score by Phenotype - Model 3") +
...
legend("right", legend = paste0("Synaptic PRS-Model 3: AUC = ", auc_value), bty="n")