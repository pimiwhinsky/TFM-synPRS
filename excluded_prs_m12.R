...
system("Rscript PRSice.R --prsice PRSice_win64.exe --base summstat-apoe.txt --target mydata-excluded --binary-target T --pheno pheno2.txt --cov synPRS-nocov.covariate --stat BETA --fastscore --no-clump --print-snp --out PRSresults-m12")

##COMPARACIÓ PRS X PHENO

prs <- read.table("PRSresults-m12.best", header=T)
...
ggplot(subset_pheno_filtered, aes(x = factor(Pheno), y = PRS)) + 
  ...
  ggtitle("Boxplot of APOE PRS Score by Phenotype - Model 12") +
...
legend("right", legend = paste0("APOE PRS-Model 12: AUC = ", auc_value), bty="n")
