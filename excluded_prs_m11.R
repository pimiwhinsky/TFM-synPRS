...
system("Rscript PRSice.R --prsice PRSice_win64.exe --base summstat-apoe.txt --target mydata-excluded --binary-target T --pheno pheno2.txt --cov synPRS.covariate --stat BETA --fastscore --print-snp --out PRSresults-m11")

##COMPARACIÓ PRS X PHENO

prs <- read.table("PRSresults-m11.best", header=T)
...
ggplot(subset_pheno_filtered, aes(x = factor(Pheno), y = PRS)) + 
  ...
  ggtitle("Boxplot of APOE PRS Score by Phenotype - Model 11") +
...
ggplot(subset_pheno_filtered, aes(x = Pheno, y = PRS, fill = factor(Sex))) + 
  ...
  ggtitle("Boxplot of APOE PRS Score by Phenotype and Sex - Model 11") +
  scale_x_discrete(labels = c("Control", "LOAD"))
...
legend("right", legend = paste0("APOE PRS-Model 11: AUC = ", auc_value), bty="n")
