...
system("Rscript PRSice.R --prsice PRSice_win64.exe --base summstats.txt --target mydata-excluded --binary-target T --pheno pheno2.txt --cov synPRS-nocov.covariate --stat BETA --fastscore --print-snp --out PRSresults-m7")

##COMPARACIÓ PRS X PHENO

prs <- read.table("PRSresults-m7.best", header=T)
...
ggplot(subset_pheno_filtered, aes(x = factor(Pheno), y = PRS)) + 
  ...
  ggtitle("Boxplot of Synpatic PRS Score by Phenotype - Model 7") +
...
legend("right", legend = paste0("Synaptic PRS-Model 7: AUC = ", auc_value), bty="n")
